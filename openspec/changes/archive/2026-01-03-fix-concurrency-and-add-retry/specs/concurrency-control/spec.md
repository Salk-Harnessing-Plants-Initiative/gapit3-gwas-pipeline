...
- Job count query MUST complete in < 1 second under normal conditions
- Additional grep operations MUST NOT add > 100ms overhead
- Wait loop polling interval MUST remain at 30 seconds

### Compatibility

- Fix MUST work with RunAI CLI v3.0+
- Fix MUST handle runai output format variations (whitespace, column alignment)
- Fix MUST NOT break existing .env configurations

### Backwards Compatibility

- .env file format MUST NOT change
- Command-line interface MUST NOT change
- Error exit codes MUST NOT change
